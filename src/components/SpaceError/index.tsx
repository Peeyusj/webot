...
export default function SpaceError({ message = "Something went wrong in space.", onRetry }: SpaceErrorProps) {
  return (
    <div className="relative flex flex-col items-center justify-center w-full h-full bg-[#0f172a] overflow-hidden fade-in">
      <style>{`
        @keyframes snow {
        ...
        .space-container {
          ...
          width: 100%;
          height: 100%;
          transform: scale(0.7); /* Scaled down to fit sidebar */
      ...
      <div className="space-container">
        {[1, 2, 3, 4].map((boxNum) => (
          <div key={`box-${boxNum}`} className={`box-of-star${boxNum}`}>
            {[1, 2, 3, 4, 5, 6, 7].map((starNum) => (
              <div key={`star-${boxNum}-${starNum}`} className={`star star-position${starNum}`}></div>
            ))}
          </div>
        ))}
      ...
      {/* Error UI Overlay */}
      <div className="absolute bottom-12 left-0 right-0 z-20 flex flex-col items-center px-6 text-center">
        <h3 className="text-white font-bold text-lg mb-1 tracking-tight">Explorer, we have a problem</h3>
        <p className="text-slate-300 text-xs mb-5 leading-relaxed max-w-[250px]">
          {message}
        </p>
        <button
          onClick={onRetry}
          className="px-5 py-2.5 text-[13px] font-semibold text-slate-900 bg-white rounded-full hover:bg-slate-200 transition-all shadow-[0_0_15px_rgba(255,255,255,0.3)] hover:shadow-[0_0_25px_rgba(255,255,255,0.5)]"
        >
          Try Again
        </button>
      </div>
